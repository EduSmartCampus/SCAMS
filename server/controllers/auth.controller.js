...
const { queryMysql, insertStudent, insertLecturer,findLecturerByMail, findStudentByMail } = require("../MySQL/test");
...
const signup = async (req, res) => {
	try {
		...
		type=="lecturer"
			?insertLecturer(encryptedName,encryptedEmail, "lecturer", hashedPassword)
			:insertStudent(id,encryptedName,encryptedEmail, "student", hashedPassword)
...
const OTPCheck = async (req, res) => {
	...
	if (result.success == true) {
		const token = jwt.sign({ email,type,id }, SECRET_KEY, { expiresIn: "1h" });
		res.json({ token });
	} else {
