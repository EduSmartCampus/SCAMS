...
const AuthForm = ({ isSignUp }) => {
    ...
    // Reset error khi thay đổi trạng thái
    useEffect(() => {
        setError('');
    }, [isForgotPassword, userData, token, otp]);
    ...
    const handleSubmit = async (event) => {
        ...
        if (!isForgotPassword && !form.checkValidity()) {
            form.reportValidity();
            return;
        }

        if (isForgotPassword && userData && !token) {
            // Xử lý xác minh OTP
            if (!otp.trim()) {
            ...
            const response = await fetch('http://localhost:3000/login', {
                ...
                body: JSON.stringify({ email, type, forget: 1 }), // Chỉ gửi forget: 1 khi yêu cầu OTP
            });
            ...
            const requestBody = isSignUp
                ...
                : { email, password, type, forget: 0 }; // Luôn gửi forget: 0 cho đăng nhập/đăng ký
    ...
    const handleResetPassword = async () => {
        ...
        if (token && userData) {
            ...
            if (response.status === 201) {
                ...
                setTimeout(() => {
                    navigate('/');
                    setIsForgotPassword(false); // Reset form về trạng thái ban đầu
                    setUserData(null);
                    setToken(null);
                    setOtp('');
                    setNewPassword('');
